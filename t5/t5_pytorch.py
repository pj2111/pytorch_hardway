...
# Notes state LayerNorm is used without Bias
...
class FeedForward(nn.Module):
    def __init__(self, dim, mult = 4, dropout = 0.0):
        super().__init__()
        inner_dim = int(dim * mult)  # inner dim 
...
class T5RelativePositionBias(nn.Module):
    def __init__(self, scale, causal=False, 
    ...
    @staticmethod
    def _relative_position_bucket(relative_position, causal=True,
                                  num_buckets=32, max_distance=128):
        ret = 0
        n = -relative_position
        if not causal:
...
class T5SelfAttention(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.causal = causal
        # Query, Key and Value layer are created
    ...
    def forward(self, x, mask=None):
        ...
        q, k, v = self.to_q(x), self.to_k(x), self.to_v(x)  # Q, K, V are created
...
class T5Encoder(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.layer = nn.ModuleList([])  # there are multiple layers of encoder
        # that are collated inside the ModuleList
        for _ in range(depth):
            self.layer.append(nn.ModuleList([
                Residual(PreNorm(dim, T5SelfAttention(dim=dim,
                                                      heads=heads,
                                                      dim_head=dim_head,
                                                      causal=causal,
                                                      dropout=dropout))),
    ...
    def forward(self, x, mask=None):
        x = self.token_emb(x)  # data will enter here
        ...
        for _ in range(depth):
            self.layer.append(nn.ModuleList([
                Residual(PreNorm(dim, T5SelfAttention(dim=dim,
                                                      ...
                                                      causal=causal, 
                                                      dropout=dropout))),
                Residual(PreNorm(dim, T5CrossAttention(dim=dim,
                                                       heads=heads,
                                                       dim_head=dim_head,
                                                       dropout=dropout))),
                Residual(PreNorm(dim, FeedForward(dim=dim,
